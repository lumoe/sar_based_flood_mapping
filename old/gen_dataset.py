...
with rasterio.open(x_paths[0]) as x_src, rasterio.open(y_paths[0]) as y_src:
    ...
    x_img[x_img == x_src.nodata] = 0.0
    y_img[y_img == y_src.nodata] = 0.0
    ...
    for row in range(0, int(sar_window.height) - window_size[0] + 1, stride[0]):
        for col in range(0, int(sar_window.width) - window_size[1] + 1, stride[1]):
            sar_patch = x_img[
                :,
                sar_row_off + row : sar_row_off + row + window_size[0],
                sar_col_off + col : sar_col_off + col + window_size[1],
            ]
            reference_patch = y_img[
                :,
                ref_row_off + row : ref_row_off + row + window_size[0],
                ref_col_off + col : ref_col_off + col + window_size[1],
            ]
            ...
            if (
                (np.sum(sar_patch) != 0)
                and (np.sum(sar_patch[:, 0, :]) != 0)
                and (np.sum(sar_patch[:, -1, :]) != 0)
                and (np.sum(sar_patch[:, :, 0]) != 0)
                and (np.sum(sar_patch[:, :, -1]) != 0)
            ):
